function New-Logger {
    ...
    $dir = Split-Path -Path (Get-PSLConfigValue -Name ConfigFile)
    $dir = Join-Path -Path $dir -ChildPath logs
    if (-not (Test-Path -Path $dir)) {
        $null = New-Item -ItemType Directory -Path $dir
    }
    $logfile = Join-Path -Path $dir -ChildPath "PSLbot-$((Get-Date -Format FileDate).ToString()).log"
    $append = $true
    ...
    [log4net.LogManager]::GetLogger("PSLbot")
}