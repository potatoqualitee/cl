function New-Logger {
    ...
    $dir = Split-Path -Path (Get-PSCLConfigValue -Name ConfigFile)
    $dir = Join-Path -Path $dir -ChildPath logs
    if (-not (Test-Path -Path $dir)) {
        $null = New-Item -ItemType Directory -Path $dir
    }
    $logfile = Join-Path -Path $dir -ChildPath "PSCLbot-$((Get-Date -Format FileDate).ToString()).log"
    $append = $true
    ...
    [log4net.LogManager]::GetLogger("PSCLbot")
}